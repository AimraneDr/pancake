...
b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle) {
    ...
    if ((mode & FILE_MODE_READ) != 0 && (mode & FILE_MODE_WRITE) != 0) {
        ...
        PANCAKE_ERROR("Invalid mode passed while trying to open file: '%s'", path);
        return false;
    }
    ...
    if (!file) {
        PANCAKE_ERROR("Error opening file: '%s'", path);
        return false;
    }
...
b8 filesystem_read_line(file_handle* handle, char** line_buf) {
    if (handle->handle) {
        ...
        if (fgets(buffer, 32000, (FILE*)handle->handle) != 0) {
            ...
            *line_buf = pancake_allocate((sizeof(char) * length) + 1, MEMORY_TAG_STRING);
...
b8 filesystem_read_all_bytes(file_handle* handle, u8** out_bytes, u64* out_bytes_read) {
    if (handle->handle) {
        ...
        *out_bytes = pancake_allocate(sizeof(u8) * size, MEMORY_TAG_STRING);
