...
b8 vulkan_buffer_create(
    ...
    pancake_zero_memory(out_buffer, sizeof(vulkan_buffer));
    ...
    if (out_buffer->memory_index == -1) {
        PANCAKE_ERROR("Unable to create vulkan buffer because the required memory type index was not found.");
        return false;
    }
    ...
    if (result != VK_SUCCESS) {
        PANCAKE_ERROR("Unable to create vulkan buffer because the required memory allocation failed. Error: %i", result);
        return false;
    }
...
b8 vulkan_buffer_resize(
    ...
    if (result != VK_SUCCESS) {
        PANCAKE_ERROR("Unable to resize vulkan buffer because the required memory allocation failed. Error: %i", result);
        return false;
    }
...
void vulkan_buffer_load_data(vulkan_context* context, vulkan_buffer* buffer, u64 offset, u64 size, u32 flags, const void* data) {
    ...
    pancake_copy_memory(data_ptr, data, size);
    vkUnmapMemory(context->device.logical_device, buffer->memory);
}
