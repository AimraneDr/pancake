...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        PANCAKE_FATAL("Failed to acquire swapchain image!");
        return false;
    }
...
void vulkan_swapchain_present(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR) {
        ...
        PANCAKE_FATAL("Failed to present swap chain image!");
    }
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (!swapchain->images) {
        swapchain->images = (VkImage*)pancake_allocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views) {
        swapchain->views = (VkImageView*)pancake_allocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
