...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    VkResult result = vkAcquireNextImageKHR(context->device.logical_device, swapchain->handle, timeout_ms, image_available_semaphore, fence, out_image_index);
    ...
    if(result == VK_ERROR_OUT_OF_DATE_KHR){
        ...
        PANCAKE_FATAL("Failed to acquire swapcahin image !");
        return FALSE;
    }
...
void vulkan_swapchain_present(
    ...
    VkResult result = vkQueuePresentKHR(presnt_queue, &present_info);
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain){
    ...
    swapchain_extent.width = PANCAKE_CLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = PANCAKE_CLAMP(swapchain_extent.height, min.height, max.height);
    ...
    //set up the queue family indeces
    if(context->device.graphics_queue_index != context->device.present_queue_index) {
        u32 queue_family_indeces[] = {
    ...
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handle, &swapchain->images_count, 0));
    if(!swapchain->images){
        swapchain->images = (VkImage*)pancake_allocate(sizeof(VkImage) * swapchain->images_count, MEMORY_TAG_RENDERER);
    }
    if(!swapchain->views){
        swapchain->views = (VkImageView*)pancake_allocate(sizeof(VkImageView) * swapchain->images_count, MEMORY_TAG_RENDERER);
    }
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handle, &swapchain->images_count, swapchain->images));
    ...
    if(!vulkan_device_detect_depth_format(&context->device)){
        ...
        PANCAKE_FATAL("Failed to find a supported format !");
    }
    ...
    PANCAKE_INFO("SwapChain Created Successfully.");
}
