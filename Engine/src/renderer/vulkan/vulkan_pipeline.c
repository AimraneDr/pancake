...
b8 vulkan_graphics_pipeline_create(
    ...
    pancake_zero_memory(&color_blend_attachment_state, sizeof(VkPipelineColorBlendAttachmentState));
    ...
    if (vulkan_result_is_success(result)) {
        PANCAKE_DEBUG("Graphics pipeline created!");
        return true;
    }

    PANCAKE_ERROR("vkCreateGraphicsPipelines failed with %s.", vulkan_result_string(result, true));
    return false;
}
